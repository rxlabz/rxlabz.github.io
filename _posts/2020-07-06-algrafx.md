...
Petite rétrospective d'une expérimentation graphique de +-20 étapes en +- 20 ans

<!--more-->

Il y a près de 20 ans je me suis m'y à dessiner des fléches

![origin](https://rxlabz.github.io/img/algrafx/fleche_origin.png)

20 ans après, j'en suis là de cette expérimentation.

<video src="/img/algrafx/clips/final.mp4" controls ></video>

Voilà quelques étapes intermédiaires entre les 2.
...
Et c'est en applicant ces concepts à l'utilisation du canvas, que j'ai eu une idée d'implémentation pour la génération de mes flèches !

L'idée était finalement simple : un canvas "génératif" interactif transformant une suite de positions du curseur en polygones "flèchés" : `points => polygones => fleches`
...
<video src="/img/algrafx/clips/rx_codepen.mp4" controls ></video>

____

Tout cela nous amène à l'été 2020, au programme : dessiner et animer des flèches dans un canvas Flutter !
...
[`CustomPaint`](https://api.flutter.dev/flutter/widgets/CustomPaint-class.html) nous donne accès à la couche de painting, et va nous permettre de manipuler un [`Canvas`](https://api.flutter.dev/flutter/dart-ui/Canvas-class.html), offrant les méthodes habituelles de dessins : `moveTo`, `lineTo`, `drawRect`, `drawCircle`...
 ...
 La responsabilité de manipuler le canvas est délégué à une instance de [`CustomPainter`](https://api.flutter.dev/flutter/rendering/CustomPainter-class.html), classe qu'il nous faut étendre pour peindre nos propres instructions. 
...
Pour cela, au lieu d'émettre seulement un point, nous pouvons cumuler les positions successives du curseur.  
...
Pour animer le tracé, on peut appliquer une *pseudo-force* à ses points.

Pour cela, plutôt que de manipuler seulement des `Offset`, on peut créer une entité `Point`, sur laquelle on appliquera un déplacement proportionel à la force appliquée et subissant une légère accélération.
...
```
Pour rafraichir le dessin nous utilisons un controller d'animation "infinie" ( [`AnimationController.unbounded`](https://api.flutter.dev/flutter/animation/AnimationController-class.html) ) , pour mettre à jour les points, et donc le canvas, à chaque *tick*.
...
``` 
Pour finir, redessinons les paralèlogrammes.
...
<video src="/img/algrafx/clips/step8a.sd.mp4" controls ></video>
...
<video src="/img/algrafx/clips/step8b.sd.mp4" controls ></video> 

## Etape 9 : snapshot

<video src="/img/algrafx/clips/step9.sd.mp4" controls ></video>

Nous allons ensuite permettre la "fixation" des polygones.

La fixation pourrait être déclenché manuellement, comme c'est le cas avec algrafx, mais dans cette exemple nous allons figer les segments visibles automatiquement, à intervalle régulier.
...
<video src="/img/algrafx/clips/step10.sd.mp4" controls ></video>
...
<video src="/img/algrafx/clips/algrafx_gradient.mp4" controls ></video>
...
<video src="/img/algrafx/clips/algrafx_step12.mp4" controls ></video>
  ...
  canvas.drawPath(
    ...
    Paint()
      ..shader = ui.Gradient.linear(
        ...
        [
          segment.fillColor
              .lighter(darkerFactor) /*.withOpacity(globalOpacity)*/,
          segment.fillColor /*.withOpacity(globalOpacity)*/,
          segment.fillColor
              .darker(darkerFactor) /*.withOpacity(globalOpacity)*/,
        ],
        [.0, .3, .8],
      ),
  );
...
```
<video src="/img/algrafx/clips/algrafx_z-order.mp4" controls ></video>
