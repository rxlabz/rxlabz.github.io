...
je colori(s)e ensuite avec Photoshop ou Illustrator.
...
je continue à développer des applications Flex ( je ne voyais rien qui me permette de développer des applications web "riches" aussi vite ).
...
Tout cela nous amène à l'été 2020, au programme : une implémentation pas à pas de ce genre d'animations.
...
## Etape 0 : L'origine
 ...
 Les plus grands voyages débutent par un 1er pas, ici nos lignes débuteront par un 1er point, ou plus exactement un rond, d'abord placé en (0,0).
 Le dessin sur le canvas étant statique `shouldRepaint` renvoie `false`.
...
Maintenant plaçons le cercle au centre de la fenêtre.
...
Pour cela, plutôt que de manipuler seulement des `Offset`, on peut créer une entité `Point`, sur laquelle on appliquera un déplacement proportionel à la force appliquée.
...
Pour "fluidifier" la forme, nous allons transformer les parallélogrammes en trapèzes. Chaque trapèze aura une épaisseur "en entrée" et une épaisseur en sortie.
...
- jouer avec du flou
 ...
 <video src="/img/algrafx/clips/final.mp4" width="640" height="480" controls preload></video>